...
MESSAGE: Hi [Name], I came across your profile...`;
  ...
  await test('Chat ID authorization check works correctly', () => {
    ...
    const isAuthorized = (chatId: string) => chatId === authorizedId;
  ...
  await test('Email guessing from name + domain', () => {
    const guessEmail = (firstName: string, lastName: string, domain: string) => {
    ...
    assertEqual(
      guessEmail('Priya', 'Sharma', 'razorpay.com'),
      'priya.sharma@razorpay.com',
      'Should guess email correctly'
    );
    assertEqual(
      guessEmail('', 'Sharma', 'razorpay.com'),
      null,
      'Should return null for missing first name'
    );
  });
